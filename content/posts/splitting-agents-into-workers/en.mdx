...
The first orchestrator had four workers: web search, coding, query preprocessing and image generation. I sent "what's Samsung Electronics' share price today," and the LLM chose web search, but at first the answer came back empty. The gateway call that returns the worker's result to the LLM got a 400 (the matching `tool_use` wasn't there before the `tool_result`). With that fixed, the sandbox streamed 169 characters and the screen showed zero. Only after fixing that too did the answer come out: ₩261,000.
...
Asked about an attachment, the orchestrator called the large-attachment analysis agent we'd built separately for that purpose, one that reads documents in map-reduce chunks. The answer was 36 characters: "there is no attached document I can read." That agent's card said:
...
With the workers in that shape, I asked for three things together. Find the number of steps in one section of the attached document, find what the state machine's components are in the internal docs, check today's weather in Seoul on the web, and finally work out in code the number of steps times seven.
...
| Case | Symptom                                  | Kind                       | Fixed in              |
| ---- | ---------------------------------------- | -------------------------- | --------------------- |
| A    | Forgot the previous turn and asked back  | not called                 | orchestrator history  |
| B    | Said "not found" after an empty catalog  | not called                 | prompt                |
| C    | Called the retriever with no index       | argument                   | tool argument         |
| D    | Called a worker without passing the file | worker the card warned off | new worker + file arg |
| E    | Kept calling the catalog, then gave up   | not called                 | catalog worker        |
| F    | 2024 date in the query                   | argument                   | prompt + code         |
| G    | Picked a test index                      | argument                   | catalog worker        |
| H    | Didn't re-search on the same question    | not called                 | prompt + code         |
...
Each fix landed on one worker or one spot in the prompt. When the index catalog acted up, I looked only at the catalog; when attachments failed, only at the attachment worker. In an agent written as a state machine, everything was tied into one flow, and pulling things apart like this was hard.

Card quality, the thing the design worried about most, wasn't a problem in this period's record. Every card was drafted and registered by an agent. My part was creating the empty agents, handing over their ids, and deciding which workers there should be. As of the 22nd, four cards don't match what their workers actually do. The retriever and index catalog cards still describe them as they were before the index became required and the LLM came out, and the other two are about how sources get numbered and a worker's old name. None of the four has a record of causing a wrong-worker pick. The retriever card being out of date was one of the reasons case C called it without an index.
...
There are reasons not to take this at face value, though. These were dev-cluster conversations, and every card was written by an agent following the same template. Which model ran each turn mostly wasn't recorded, and a few conversations don't record which worker was called. The orchestrator is also a version written first as sandbox code. When the next stage moves this loop into the chat service's tool loop, the chat service will attach history, so I'd expect case A to go away on its own, but we'll only know once it's moved.
