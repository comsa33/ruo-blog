...
<Sequence
  ...
  steps={[
    { from: 'Upstream', to: 'Commit 1', label: 'not one line changed', note: 'The original, verbatim. This commit is the reference point. Mix anything else in and the reference point is gone.' },
    { from: 'Commit 1', to: 'Commit 2', label: 'path rewrites only', note: 'Import paths and nothing else. 82 of them. Mechanical, so it needs no review and can be ignored in later diffs.' },
    { from: 'Commit 2', to: 'Commit 3', label: 'our changes', note: 'Only now do the real modifications land. The diff of this commit is exactly "everything we differ from upstream by".' },
    { from: 'Commit 3', to: 'Commit 3', label: 'and everything after', note: 'Later changes stack the same way. Mix 1 and 3 and you permanently lose the ability to tell original from ours.' },
  ]}
/>
...
<Transform
  ...
  variants={[
    ...
    },
    {
      ...
      verdict: 'Every one of them hands a synchronous function to a thread. Nowhere in the store or embedding adapters is there an async function or an async HTTP client.',
      bad: true,
    },
  ]}
/>
