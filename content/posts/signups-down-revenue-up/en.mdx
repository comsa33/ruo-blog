...
That's the set of principles I wrote down on September 20th. None of it has been tested even once yet. The next peak is March–April, so the result won't be in for half a year. If it turns out then that assets built in the off-season raised peak signups, this post was right. If not, it's a post that used seasonality as an excuse to let six months slide.

The seasonal curve is someone else's data, too. Right now Google Trends is the reference, and whether our own signup curve shows the twice-a-year spike will take another peak to find out. I've decided to log the three series together once a month, and I think that after the second peak passes we'll be able to read the phase without Trends.
