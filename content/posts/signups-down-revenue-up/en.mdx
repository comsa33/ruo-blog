...
topic: 'Demand seasonality'
date: 2026-09-21
tags: ['Cohorts', 'Search demand', 'Decision-making']
draft: false
---
...
The embarrassing part is that I already knew this trap. I hit exactly the same thing writing a cost-aggregation query for the same app, and I wrote it down in my notes at the time. Turns out a note only works if you read it. Only after fixing the query did the real number come out.
...
<Callout label="Right after a peak, everything looks like an incident" accent>
  With blade-shaped demand, the week after the peak is always a plunge. If you look only at your own
...
The most expensive item on this table is the payment check during the rise. Right before this September's peak, Android in-app payments were silently dead for ten days, and [I wrote about that separately](/en/everything-but-the-payment-test). At the time it ended as "I didn't verify". Seen from here, those ten days sat on the threshold of one of only two revenue windows in the year. The same incident costs a different amount depending on when it happens, and in this business, right before a peak is when it costs the most.
