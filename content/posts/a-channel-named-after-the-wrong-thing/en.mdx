...
By its symptom this is a streaming bug, so the streaming code is where I looked first. Buffering, chunk boundaries, reassembly. None of it. **Duplication isn't a pipe problem but a subscriber-count problem.** One writer and two readers emit twice however well the pipe is written. While you are looking at the pipe, you can't see that.
...
Tokens weren't repeating — they were being spliced into each other's positions. While streaming, a citation goes out as a placeholder first, and once the source is resolved an event follows saying "replace this string with that one". Two parallel requests shared one channel, so those replacements landed in each other's text. They applied where a replacement had already happened, and partial matches accumulated.

The symptoms look nothing alike; the cause is one line. The channel is named after the request ID, and the request ID wasn't unique per stream. Nested, it echoes. Parallel, it interleaves.

## Two fixes, one shape

The first thing shipped was a stopgap: move publishing out of the agent level and up to the orchestrator level. Remove the publisher on the inside and the loop can't close. The symptom stopped; the channel name didn't change.
...
Sandbox code has no contract. The file behind this incident is user code, one per agent, living outside version control. Checking it now: it forwards the agent-name headers but still passes the original request ID. The shared client handles all thirteen headers for you, and nothing blocks the path around it that hand-rolls an HTTP client. I don't think the gap is really knowledge; it's that the bypass stays open.
