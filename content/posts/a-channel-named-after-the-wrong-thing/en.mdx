...
<Sequence
  ...
  steps={[
    { from: 'Browser', to: 'Chat service A', label: 'query', note: 'A subscribes to sse.response:ORIG. Correct so far.' },
    { from: 'Chat service A', to: 'Sandbox', label: 'run code', note: 'The user-written code starts running. Progress messages are published by the sandbox onto that same channel.' },
    { from: 'Sandbox', to: 'Chat service B', label: 'call another agent · same request ID', note: 'The ID is passed through as-is. B builds its own channel name from it — sse.response:ORIG. The channel A is already listening to.' },
    { from: 'Chat service B', to: 'Sandbox', label: 'token "print"', note: 'B receives a token from the model and returns it to the sandbox, which publishes it to the channel.' },
    { from: 'Sandbox', to: 'Chat service B', label: 'reads back what it wrote', async: true, note: 'B is a subscriber too. It receives the event its own answer caused, and streams that out as response. The sandbox publishes again.' },
    { from: 'Sandbox', to: 'Browser', label: 'the same token, over and over', async: true, note: 'A is listening to the same channel, so every repetition goes to the browser.' },
  ]}
/>
