...
The agent wasn't wrong. `minifyEnabled` and `shrinkResources` are the options Google asked for, turned on the way the documentation says to turn them on. What was wrong came after. Whoever writes the code, pressing the revenue path once before shipping is the operator's job, and I didn't do it.
...
When the first fix is logically right and the symptom doesn't move, I think the thing to try is suspecting a second cause before abandoning the hypothesis.
...
The loss itself is small. Eleven app payments in August, 69,300 KRW, became zero in September. It isn't the amount I mind. One revenue path was 100% dead and for ten days no instrument mentioned it.
...
The part that sticks with me is this one. When an agent does well for long enough, the checking quietly loosens. I had replaced a per-release check with "it has been fine so far," and the day that substitution first failed was the day I changed the build configuration. There was a store deadline pushing too, and a release with a deadline is exactly the one that should go through the internal test track, but the judgement went the other way. Whoever writes the code, **deciding what to verify before shipping isn't something you can hand over.**

A web app in a native wrapper needs one more layer of attention. Capacitor's own plugins ship their keep rules with the library, so they are protected. Cordova-compatible plugins aren't. Normally only the web is deployed, so it is easy to forget that layer exists at all — until the day the native build is touched and the whole bill arrives at once.
