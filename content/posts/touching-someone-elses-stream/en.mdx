...
<Transform
  ...
  variants={[
    ...
    },
    {
      ...
      verdict: 'The regex only accepts lines beginning with id, event, data or retry. A comment frame starting with a colon is valid SSE but never matches, so it disappears silently. Heartbeats are exactly that.',
      bad: true,
    },
    {
      key: 'v5',
      ...
      verdict: 'Any frame without the string data: is discarded. Frames carrying only a status notice or an identifier never reach the client.',
      bad: true,
    },
  ]}
/>
...
That version was not a wrong choice. It was **a different product.** Keeping it would have made us a transparent proxy and pushed the job of absorbing provider differences onto every client. What cannot be undone is not the code — it is that promise.
</Callout>
...
<Structure
  ...
  nodes={[
    { id: 'tpl', label: 'Generation template', sub: 'a string', col: 0, row: 0, accent: true, note: 'The relay logic sits here inside a string. That is why the braces are doubled — this file is not executed code, it is a mould that stamps out code.' },
    { id: 'gen', label: 'Generated api.py', sub: 'one per provider', col: 1, row: 0, note: 'One file is produced for each registered external provider. The relay logic is copied into every one of them.' },
    { id: 'sub', label: 'Sub-app', sub: 'mounted', col: 2, row: 0, note: 'The router extracted from the generated file is attached here. Only now is it callable.' },
    { id: 'regen', label: 'Regenerate all', sub: 'maintenance API', col: 1, row: 1, note: 'Fixing the template does not touch files already produced; they keep the old code. So a dedicated endpoint exists purely to stamp every deployed file again.' },
  ]}
  edges={[
