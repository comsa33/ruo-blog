...
<Sequence
  ...
  steps={[
    { from: 'Agent', to: 'Memory service', label: 'give me preferences', note: 'First turn of the morning. Nothing has touched this path since last night.' },
    { from: 'Memory service', to: 'Connection pool', label: 'borrow', note: 'The pool hands over a connection it opened last night. It does not check whether it is alive.' },
    { from: 'Connection pool', to: 'Database', label: 'query', async: true, note: 'The server closed this connection hours ago. Only the pool was unaware.' },
    { from: 'Database', to: 'Memory service', label: 'connection is lost', note: 'An exception is raised here. The problem is what happens next.' },
    { from: 'Memory service', to: 'Agent', label: 'empty list · 200', note: 'The rule was that failing to load memories must never block the conversation. So the failure is swallowed and an empty result returned. The user gets a 200, and the agent answers without knowing who it is talking to.' },
  ]}
/>
...
<Transform
  ...
  variants={[
    ...
    },
    {
      ...
      verdict: "SQL's three-valued logic. NULL is neither true nor false, so it cannot pass the condition. Excluded without being an exclusion target.",
      bad: true,
    },
  ]}
/>
...
In this window the conversation is still **the previous day** in UTC. So "tomorrow" resolves to today and "yesterday" to the day before, and that resolution is **stored permanently.** A user says "the meeting tomorrow" at half past midnight, and the memory records a meeting today.
</Callout>
...
<Metrics>
  <Metric value="full scan" label="scope queries never used an index. The filter key was bound as a parameter, so it could not match an expression index" />
  <Metric value="TEXT" label="ledger timestamps were strings. The moment offsets mix, lexical order stops matching chronological order" />
  <Metric value="no ORDER BY" label="LIMIT without a sort. The same query returns a different page every time" />
</Metrics>
